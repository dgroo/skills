...
4. Ask via `AskUserQuestion`. **Never skip this question, even for changes that look obviously trivial / mechanical / safe** — the question is not a sanity check on the riskiness of the edit, it's the user's choice on whether the recommendation itself is the right thing to do. A "small mechanical doc-comment add" can still be the wrong recommendation (wrong framing, wrong place, premature, the report's reasoning was inaccurate); the only way to find that out is to ask. Use these options:
