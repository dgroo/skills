...
**"Federation" is literal — this phase enrolls the repo in a live cross-vault sync.** `~/bin/obsidian-config-sync` (launchd agent `com.groot.obsidian-config-sync`) sweeps `~/code` and reconciles shared vault config across every participating vault. It auto-discovers by presence of a tracked `.obsidian/`, so **committing the config this phase writes is what joins the fleet** — expect an unattended `chore(obsidian): sync shared config (…)` commit to land in the repo shortly after, authored by the watcher, not by you. That's the system working, not a bug. Say so up front; it is otherwise a genuine surprise (it was, in vffins on 2026-07-15).

The two systems compose deliberately: the `coding` group syncs `hotkeys.json`, `core-plugins.json`, and generic snippets, while `SNIPPET_EXCLUDE = {"project-color.css"}` keeps this phase's per-project identity color from being flattened fleet-wide. **Don't "clean up" that exclusion** — it is the seam that lets shared config and per-project color coexist.
...
**Curated plugin set** (each maps a real workflow): `dataview` (query `design/stories/` by frontmatter), `templater-obsidian` (dynamic `STORY_TEMPLATE`), `obsidian-style-settings` + `obsidian-minimal-settings` (drive Minimal + snippet controls), `obsidian-git` (change list / commit / diff panels) + `git-file-explorer` (colors modified+untracked files in the file tree — the highlight half, since obsidian-git is panels-only). The git pair earns its place when a vault's `design/` is live-synced: cross-host dirt shows up at a glance. `obsidian-git` ships a `data.json` pinning every background interval to 0 — these vaults are roots of _active code repos_, so commits stay a deliberate button-press and can't fight the design-sync watcher. **`obsidian-setup.py`'s `PLUGINS` dict is the source of truth for this list** — if you change it, update this line in the same commit (they drifted 4-vs-6 once already).

Versions are **pinned to the newest release that runs on stable Obsidian** — not "latest", which is a trap: Templater 2.21+ requires Obsidian 1.13.0 (an insider beta), so a "latest" install would download a plugin that silently refuses to enable. Bump the pins in `obsidian-setup.py` deliberately, re-checking each `minAppVersion` against current stable.
