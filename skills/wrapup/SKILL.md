---
name: wrapup
description: Actively assess whether this session is safe to end. Inventories in-flight state, executes routine wrap-prep writes (DIARY entries, story files, TODO.md ## Next up handoff, queue notes) directly, auto-commits and pushes complete work (surfacing only ambiguous or destructive cases), then gives a definitive READY / WAIT / STAY verdict. With an intent hint, also judges whether THIS session is the right place for that next chunk and pushes back if it is.
...
| ------------ | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/wrapup ?`  | **Advise only.**     | Inventory + Phase 4 recap + verdict. **Makes no changes** — no auto-`/cpush`, no DIARY / story / `## Next up` / queue writes. "Where do we stand?" Overlaps `/sup`'s readiness read by design; it's the lightest touch.                                                                                                                                                                                                              |
...
- **In-flight todos worth not losing** — route by shape:
  - **The next session's ordered priorities** (the "next: X, then Y" you'd otherwise only say in the tl;dr) → write them into a **`## Next up` section at the top of `TODO.md`**, in priority order, as `- [ ]` checkboxes; create the heading directly above `## Open` if absent. This is the cross-session handoff: `/sup` and `/next` read `## Next up` and surface those items _first, in this exact order_ (via `backlog-scan`'s pinned surface + [`backlog-ranking.md`](../next/backlog-ranking.md) §3 rule 0), so the order you anoint here is the order the next session picks up — it survives the `/clear` that discards the tl;dr and scrollback. This is the **producer half** of that handoff; writing it is the whole point of `/wrapup` deferring work. Keep it tight (the few things that genuinely come next, not a backlog dump); an item moves to `## Done` when it ships. **`## Next up` is the single producer/consumer constant — that exact heading string, or the convention silently breaks.**
...
The READY verdict's contract: after the user bounces and runs `/sup` in a fresh session, the new session has everything it needs. That's why journal-completeness, `## Next up` currency, and "uncommitted decisions" matter as inventory items — they're the things `/sup` can't recover if `/wrapup` didn't catch them first.
