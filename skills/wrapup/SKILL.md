...
6. **Cadence table** — run `cadence-nudges` (a read-only dotfiles wrapper over the silent-unless-due helper family: `last-codex-review` >21d since an independent Codex look with code changed since; `backlog-staleness` >30d untended stories, nudges toward `/pm`; `last-beginners-mind` >60d since a fresh-eyes audit, onboarded projects only; `goals-block-check` missing/stale `## Project goals` block in CLAUDE.md). Prints a `⏰ Cadence` header plus one `↳` line per due ceremony, nothing when none are due. Periodic codebase-wide reminders (Derek won't remember on his own), **not** per-session gates. `/sup` runs the same wrapper at session start; this end-of-session run is the backstop. Capture output for Phase 4; if the wrapper is absent (older host / dotfiles not pulled), fall back to any individually-present helpers' `--nudge` forms — never block.
...
If Phase 1's cadence table (step 6: `cadence-nudges`) produced any nudge line(s), surface the block **between the recap and the verdict block** — non-blocking context, never part of the ✅/⏸/↺ judgment. Verbatim from the wrapper; don't expand (they're "maybe, sometime" reminders Derek may skip). Every helper is silent unless due, so most wraps show none.
